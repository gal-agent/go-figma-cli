...
description: Inspect Figma files via go-figma-cli: pages, node tree, variables, screenshots. Load when user mentions or agent finds task highly matches check figma node, design tokens, figma 截图, 看下设计稿, 变量查询.
---

# Figma inspection (go-figma-cli)

Answer questions about a Figma design **without generating code**: structure,
tokens, visual reference. Syntax lives in `figma --help` (every command has
